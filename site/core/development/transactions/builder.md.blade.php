...
Now that we have implemented our builder class, we can use it to build new custom transaction payloads. The [code snippet](https://github.com/learn-solar/dapp-custom-transaction-example/blob/master/__tests__/test.test.ts#L13-L17) below shows us how to use the TransactionBuilder to create signed and serialized transaction payloads.

<x-alert type="danger">
When using Transaction Builder on the client side (your mobile/web app) make sure to register your TransactionType inside the **@solar-network/crypto** package with the following code:
