...
You might be used to tab completion, which the SXP CLI does support. Using this command SXP CLI but does show you instructions.
...
```bash
solar config:publish --network=mainnet --reset
```
...
```bash
solar env:get --key=CORE_LOG_LEVEL
```
...
```bash
solar env:set --key=KEY --value=VALUE
```
...
```bash
solar env:set --key=CORE_LOG_LEVEL --value=info
```
...
```bash
solar plugin:install {npm|git|file}
```

##### NPM

```bash
solar plugin:install @vendor/pkg
```

##### Git

```bash
solar plugin:install git@github.com:vendor/pkg.git
```

##### Usage

```bash
solar plugin:install /path/to/some/vendor/pkg.tar.gz
```
