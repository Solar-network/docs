...
<x-alert type="info">
This guide is for setting up the development environment with Docker. If you are looking for Solar Core Production ready Docker images, they are now available at [Docker Hub](https://hub.docker.com/r/arkecosystem/core), but are not meant to be used for development purposes.
</x-alert>
...
Solar Core includes several `Dockerfile` and `docker-compose.yml` templates to ease development. They can be used to generate different configurations, depending on the network and token.
...
2. [Run a PostgreSQL container, build and run SXP-Core using a mounted volume](#approach-2-serve-core-node-and-postgres-as-a-collection-of-containers)

## Approach 1: Containerize Only the Persistent Store

**This approach runs a PostgreSQL container while using NodeJS from your local environment.** This configuration is well suited when you are not developing Solar Core, but instead working with the API. By tearing down the PostgreSQL container, you reset the Nodes blockchain.
...
```bash
docker logs solar-$NETWORK-core -f
```
...
<x-alert type="danger">
Development files/presets are not Production ready. Official Production SXP-Core Docker images are now available at [Docker Hub](https://hub.docker.com/r/arkecosystem/core).
</x-alert>
