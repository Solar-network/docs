...
It's possible to use your favorite Windows IDE locally and build, test and run Solar Core in a native Linux environment. This can be done by using [Docker for Windows](https://docs.docker.com/docker-for-windows/).
...
Once you installed [Docker Desktop for Windows](https://docs.docker.com/docker-for-windows/install) you'll have to configure _**File Sharing**_ so your Linux container can mount Solar Core repository cloned locally on your Windows system. Make sure your **firewall** and **antivirus** are turned off or configured to allow the above operation. Follow [Docker Documentation](https://docs.docker.com/docker-for-windows/) - section **FILE SHARING**.
...
Prior to cloning Solar Core repository, enter the following code in your command prompt or PowerShell:
...
### Step 4.2: [Optional] Adjust Solar Core PATH

<x-alert type="info">
If you cloned Solar Core to any other PATH different than**`D:\core`** you'll have to edit `docker-compose.yml` to match the path:
</x-alert>
...
```bash
docker exec -it solar-testnet-core bash
```

**You can now build and run Solar Core from inside the container. Example:**
