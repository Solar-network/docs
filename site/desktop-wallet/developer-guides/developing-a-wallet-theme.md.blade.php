...
This tutorial walks you through everything you need to know to create a beautiful custom theme plugin in around one hour.  We’ll show you how to build, customize and submit your very own color theme for approval as a plugin for the SXP Desktop Wallet Plugin Manager. All you’ll need is a little bit of creativity and around one hour to spare.
...
You will also need to install the [Desktop Wallet](https://solar.io/wallet) if you haven’t already.
...
* The next step is to use the template we have prepared to get you started. In order to do it go to this link **[https://github.com/solar-ecosystem-desktop-plugins/theme-template](https://github.com/solar-ecosystem-desktop-plugins/theme-template)** and press the ‘_**Use this template**_’ button.

<livewire:embed-link url="https://github.com/learn-solar/desktop-wallet-plugin-theme-template" caption="Desktop Wallet Theme Plugin Template Project" />
...
`"description": "Desktop Wallet Theme Template",` — a short description of the plugin, in our case this can be just something generic like what color tone you are using eg. `"description": "`_**`Pinkish SXP Desktop Wallet theme`**_`",`
...
`"logo": "https://raw.githubusercontent.com/solar-ecosystem-desktop-plugins/theme-template/master/logo.png",` — is the thumbnail that will appear inside Plugin Manager for your theme. If you are going to be doing a custom logo, it should link directly to your custom image where it needs to be named `logo.png` and be `140x140px` in size. If you are not going to be designing your own logo for your plugin then you can delete this logo line (or leave it unchanged, but we recommend deleting it so you can see how it looks without in the file below).
...
`"bugs": { "url": "https://github.com/solar-ecosystem-desktop-plugins/theme template/issues" },` —this is a link where people can report issues with your plugin. This can be just linked to your GitHub repository of theme, eg. in our case `"bugs": { "url": "https://github.com/boldninja/theme-template/issues" },`

`"homepage": "https://github.com/solar-ecosystem-desktop-plugins/theme-template#readme",` — the website for your plugin. We are just going to link it to our GitHub repository `"homepage": "https://github.com/boldninja/theme-template",`

`"url": "git+https://github.com/solar-ecosystem-desktop-plugins/theme-template.git"` — under repository, we have a direct Git link to your plugin (in our case GitHub) so we replace it the same way as the previous option with our homepage by appending .git at the end `"url": "git+https://github.com/boldninja/theme-template.git"`
...
As you can see we edited `.theme-plugin-light-mode` to what we defined in _index.js,_ so we changed it to reflect those changes. In our case, it becomes `.theme-plugin-pinky-mode` so the index.js file knows what class to read (if we don’t change it here changes won’t come into effect once we publish the plugin). After this is done, let's fire up the SXP Desktop Wallet.

Once you start the SXP Desktop Wallet click on the _**Toggle Developers Tools**_ option under the _**View**_ **menu.**
...
Once you have done this, your updated package will be ready. But remember, if you run into any kind of trouble developing this or get stuck at any step, don’t hesitate to [contact us](https://solar.dev/contact) or join the [ACF's](https://arkcommunity.fund) [Community Discord](https://discord.gg/arkecosystem) and ask for some assistance in the \#help channel.

## **Developing Your Own Plugin**

But don’t stop there… now that you’ve mastered the art of developing a theme plugin for the SXP Desktop Wallet, why not get going with a different type of plugin? If you have an idea for a game, a helpful tool or even something to improve the ecosystem then get building. With the new and intuitive Plugin Manager, soon your plugin could be discovered and enjoyed by everyone. How cool is that?
...
Plus, check out what others have been building by browsing the Plugin Manager in the [SXP Desktop Wallet](https://solar.io/wallet). Get inspiration and start bringing your idea to life. We can’t wait to see what you come up with!
