...
1. The `core:testnet` command is run within `core`, which as of the time of writing executes the following command in `npm`: `cross-env CORE_PATH_CONFIG=./bin/config/testnet CORE_ENV=test yarn solar core:run --networkStart`
...
This last step is where the meat-and-potatoes of Solar Core is loaded. During this step, the Postgres database is set up, all SXP-specific tables and fields are migrated, the genesis block is created, 51 forging delegates are created and set up to forge blocks — all the blockchain goodness you would expect from of a fully-formed testnet.
