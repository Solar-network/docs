...
We will create a new user `solar` and add this user to the `sudoers` group (allowing root execution if needed). You can skip this step as a developer and continue to next steps below.
...
As the most popular version control software in existence, Git is a staple of many developer workflows, and SXP is no exception. Downloading Git will allow you to clone the latest version of Solar Core.
...
As Solar Core is written exclusively in [Node.js](https://nodejs.org/), the server-side framework for JavaScript, Typescript, installing Node.js is a necessity for core development. The code below installs Node.js from source.
...
[Yarn](https://yarnpkg.com/) is a package manager that seeks to build upon the foundation of Node's npm. Although yarn is not a strict requirement, in many cases it works faster and more elegantly than npm. Most SXP developers use yarn, and as such, you will see yarn commands often used throughout our documentation.
...
Solar Core stores all the blockchain data in a [PostgreSQL](https://www.postgresql.org/) database. You have two options on how to setup your development database.
...
If you are already using `Docker` and  have  `docker-compose` installed, then you can generate docker files from the command line, with the `yarn docker solar` command where (solar is the name of the `network` for which you want to generate docker files). For now let's stick with `solar` as the default name of the network.

Executing the command `yarn docker solar` in the root folder of the previously cloned repository, like this:
