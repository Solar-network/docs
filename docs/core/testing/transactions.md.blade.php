...
<x-alert type="info">
The next set of transaction testing requires the use of the core-tx-tester as entity transactions are not currently supported in the SXP Desktop Wallet. To install and setup the `core-tx-tester`, follow the install guide [here](https://github.com/ArkEcosystem/core-tx-tester).
</x-alert>

[AIP-36 (Entity Declarations)](https://github.com/ArkEcosystem/AIPs/blob/master/AIPS/aip-36.md) are a new set of Magistrate Transaction-types that replaces & improves the current Business & Blockchain transactions first introduced on the SXP Public Network (APN) with the release of Solar Core v2.6.
