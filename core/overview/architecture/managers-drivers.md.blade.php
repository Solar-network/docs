...
Solar Core makes use of a variant of the Builder pattern, known as the Manager pattern, to provide a high degree of extensibility together with a delightful developer experience through expressive and simple to understand code to create your own set of drivers to alter how Solar Core behaves for certain features.

> Throughout this article we will reference the Builder pattern as Manager pattern as Solar Core internally uses the wording `Manager` for all the classes that manage the state of drivers.
...
What the Manager pattern allows you to do is to simply create a new driver, for example `RedisDriver`, based on a given implementation contract, register it with Solar Core and reap the benefits of increased performance.

**This is extremely useful for extending or altering how Solar Core behaves and what it is capable of doing, all done through the use of managers and drivers with a common public API.**
...
Solar Core comes with a variety of managers out of the box such as `CacheManager`, `LogManager` and `ValidationManager`. Lets learn how to create your own manager and how to register to expose it to other plugins.
...
Implementing a logger driver is as simple as implementing a manager. The logger contract that is provided by Solar Core follows the log levels defined in the [RFC 5424 specification](https://tools.ietf.org/html/rfc5424). Again we'll setup some boilerplate and then break it down.
