...
This section provides definition of common cryptography terms used through the Solar Network product landscape. You will learn the importance of your passphrase and how it translates to private key, public key and wallet address generation. We also explain some common encryption algorithms and encoding protocols.

## Passphrase

<x-alert type="danger">
The **passphrase** is the master password (key) for your SXP tokens. Every SXP address has its own unique passphrase. With the passphrase you can sign transactions to send your SXP or vote for a delegate.

Do not lose it, and do not share it with others, or you could lose access to your SXP tokens. If you lose your passphrase, or if it is stolen, there is nothing we can do to help you. **We CANNOT recover any lost passphrases.**
</x-alert>

A passphrase is a "key to the castle." It is used to directly calculate the [PrivateKey](#privatekey) of an SXP account and should never be shared, stored irresponsibly, or transmitted over the internet. The only person that should ever have access to a passphrase is the owner of its account.

We can technically use any word, phrase, or string as a passphrase which will result in a valid SXP [Address](#address--wallet) or Wallet; however, it is heavily discouraged as the security of an address relies on the randomness of its Passphrase. Humans are bad at creating randomness, and entering sequences of random letters and numbers isn't easy to do accurately.

To promote usability while also maintaining security, SXP passphrases are implemented using the [BIP39 Protocol](https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki). Simply, it's a mnemonic sentence constructed via randomly chosen words from a large [wordlist](https://github.com/bitcoin/bips/blob/master/bip-0039/bip-0039-wordlists.md). From here, that sentence or "Passphrase" is piped through a series of hashing, curve, and encoding algorithms to derive a [PrivateKey](#privatekey) / [WIF](#wif-privatekey), a [PublicKey](#publickey), and subsequently [Addresses](#address--wallet) / Wallets and [Signatures](#signature).

![Cryptography overview](/core/assets/overview_nologo.svg)

The following examples will be using test-fixtures from the [Solar Core](https://github.com/ArkEcosystem/core/) repo on [GitHub](https://github.com/ArkEcosystem/).
...
![PrivateKey Generation Steps](/core/assets/private_key_nologo.svg)
...
![PublicKey Generation Steps](/core/assets/publickey_nologo.svg)

SXP also uses "Point Compression" to obtain compressed PublicKeys that are 33-bytes in length.

## Address / Wallet

An SXP address is shareable much like an e-mail address. It is the destination to which SXP tokens can be sent, and is obtained from a [PublicKey](#publickey) via a combination of RIPEMD160 hashing and Base58Check encoding prefixed by a single network byte.

Since an address is derived from a PublicKey, that means it is also mathematically tied to a PrivateKey and Passphrase.

![SXP Address Generation Steps](/core/assets/address_nologo.svg)
...
![Signing Process Example](/core/assets/signature_msg_nologo.svg)

SXP Signatures use DER Encoding.
...
An SXP Signature is [DER Encoded](https://en.wikipedia.org/wiki/X.690#DER_encoding) using [BIP66](https://github.com/bitcoin/bips/blob/master/bip-0066.mediawiki) standards.

Upon obtaining a Signature from the ECDSA/SECP256K1 algorithm, it will first be in its raw form known as an "r" and "s" value.

![Signature: DER (X.690 : ASN.1) Encoding](/core/assets/signature_der_nologo.svg)
...
It is used to encode a [PrivateKey](#privatekey) and is also the final step to encoding an [SXP address](#address--wallet).
