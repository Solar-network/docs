...
When running an SXP node, especially a Delegate Node, you should consider your server's security as your main priority.
...
Depending which network this node is for will determine what port you open here. For mainnet use `6001`, testnet use `6002`, and testnet use `6000` and public API which is by default located on port `6003`.
...
Run the following command on your SXP node server.
...
Copy the contents of your `id_rsa.pub` file on your local machine to your `~/.ssh/authorized_keys` on your SXP node server.
