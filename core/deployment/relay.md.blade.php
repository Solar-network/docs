...
Setting up the new Solar Core has never been easier! If you want to have your own instance up and running, this guide will help you set one up in a few minutes with some basic Linux knowledge.
...
Executing this guide as the root user should be avoided. Instead, create a new dedicated user to manage SXP related software. On your server, type the following into the command line and press enter. `username` is the name you want to log in with with. We’ll illustrate it as **`adduser solar`** to create a user by the name of ‘solar’, but you can chose something else, if preferred.

```bash
sudo adduser <username> //in our example we'll create user by the name of 'solar' so we'll replace <username> with solar and run:
sudo adduser solar
```
...
## Step 6: Switch to the SXP user

While installing Ark Core, we should use the solar user that we created above and go to the base directory. To switch to it (again, it can be different than `solar` depending on your choice of user names in first step where we created the new user), run:

```bash
sudo su - solar
```

## Step 7: Running Solar Core installation script

Installing Solar Core is a straightforward process. We will use the SXP installer script that will install all of the necessary dependencies and Solar Core onto your server, and publish configuration files for it. To install, run this command (copy and paste it, this is a one line command):
...
```bash
[sudo] password for solar: <input your password for user you created>
```
...
## Step 8: Selecting Solar Core network
...
With that we have successfully installed Solar Core and published our configuration options.

## Step 9: Configuring Solar Core database

The last step of the required Solar Core configuration is to configure the database parameters. You will be presented with a prompt:
...
To start the SXP relay process, and with it the synchronization process with the SXP blockchain, we need to start the relay process with our integrated CLI:
...
<x-alert type="info">
All of the CLI commands with a description can be viewed in our [Core CLI](/docs/core/deployment/cli) documentation or by running the `solar help`command.
</x-alert>

## Step 11: Checking that Everything is Working

Now we want to see if the SXP Relay process has started the synchronization process. You can do that by running one of these two commands:
...
Once the syncing finishes you will see messages ‘Delegate allowed to forge’ about every 8 seconds. You can always check the current height of the blockchain on our [Explorer](https://explorer.solar.io/).

<x-alert type="info">
Extra security guide: if you want to secure your server with additional security countermeasures, please give this **"How To Secure Your SXP Node"** guide a read.
</x-alert>
